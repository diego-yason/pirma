...
|---|---|---|---|---|---|
| B1 | **Guest finalize blocked** — `sign/+page.server.ts` returns `fail(400, "Guest finalize not yet supported")`. Guests can fill the sign page but can't actually sign. *(Shared with O1-B1.)* | Functional (signing) | Guests are advertised (pricing/OTP) but can't complete the primary action. | 🔴 | Let `party.type === "guest"` finalize with an L1 session key, store against the linked recipient, validate ownership like authenticated signers. |
...
1. **B1** — guest finalize (core functional gap).
